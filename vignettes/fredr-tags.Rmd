...
FRED series are assigned __tags__ as an attribute for classification, each with a string identifiers.  For example:
...
The function `fredr_tags()` returns a list of tags matching the request along with some tag information (tag name, group ID, notes, tag creation date, tag popularity, and number of series belonging assigned the tag). For example, running `fredr_tags()` without any parameters returns the top 1000 FRED tags ordered by number of series who are assigned the tag:
...
Not that the example above search for _tags_ matching `"unemployment"`.  To search for the set of _series_ with tags matching `"unemployment"`, use `fredr_series_search_tags()`:
...
The function `fredr_related_tags()` returns a list of tags _related to_ the set of tags matching the request along with some tag information (tag name, group ID, notes, tag creation date, tag popularity, and number of series belonging assigned the tag). For example, to get the set of tags related to the tags "monetary aggregates" and "weekly":
...
The function `fredr_tags_series()` returns a list of series assigned tags matching the request along with some series information (same as the information returned by the functions for the [Series endpoint](http://sboysel.github.io/fredr/reference/index.html#Series)).  For example, to get all series tagged with "gdp":
