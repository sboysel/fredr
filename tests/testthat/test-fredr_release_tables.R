...
test_that("input is validated", {
  ...
  expect_error(fredr_release_tables(release_id = 10L, include_observation_values = 1), "`include_observation_values` must be a `logical`")
  expect_error(fredr_release_tables(release_id = 10L, include_observation_values = "true"), "`include_observation_values` must be a `logical`")
